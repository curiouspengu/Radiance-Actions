...
while restart == True:
    ...
    for i in range(len(config.config_data["potion_crafting"]["options"][potion_name]["ingredients"])):
        ...
        while True:
            ...
            if detect_ui_nav(resolution_settings["potion_crafting"]["leaderboard_close_button"].split(",")) == True:
                for i in range(2):
                    tap(Key.left)
                restart = True
                break
        ...
        if detect_ui_nav(resolution_settings["potion_crafting"]["autoroll_button"].split(",")) == True: # Recorded in 1920x1080 100%
            break
        if restart == True:
            break
